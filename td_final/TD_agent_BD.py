...
class Agent_BD():
    ...
    def voir_credits(self, nom):
        cursor = self.conn.cursor()
        cursor.execute("SELECT credits FROM joueurs_defis WHERE nom = (?)", (nom,))
        valeurs = cursor.fetchall()[0][0]
        cursor.close()
        return valeurs
    
    def ajouter_credits(self, nom, credits):
        cursor = self.conn.cursor()
        cursor.execute("UPDATE joueurs_defis SET credits = credits + ? WHERE nom = ?", (credits, nom))
        self.conn.commit()
        cursor.close()
    ...
    def ajouter_aux_usagers_locaux(self, nom):
        ...
        cursor.execute("INSERT INTO joueurs_defis (nom) VALUES (?)", (nom, ))
        self.conn.commit()
        cursor.close()
